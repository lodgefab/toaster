...
export default class NotionService {
  ...
  async getPublishedBlogPosts(): Promise<BlogPost[]> {
    ...
    return response.results.map((res) => {
      ...
      return JSON.parse(
        JSON.stringify(NotionService.pageToPostTransformer(res))
      );
    });
  }
  ...
  async getSingleBlogPost(slug: string): Promise<PostPage> {
    ...
    //Date 型を string に変換しています。これをしないと下記のようなエラーが発生します。
    post = JSON.parse(
      JSON.stringify(NotionService.pageToPostTransformer(page))
    );
    return {
      post,
      markdown,
  ...
  async getPublishedProjectPosts(): Promise<ProjectPost[]> {
    ...
    return response.results.map((res) => {
      //Date 型を string に変換しています。これをしないと下記のようなエラーが発生します。
      return JSON.parse(
        JSON.stringify(NotionService.pageToProjectPostTransformer(res))
      );
    });
  }
  ...
  async getSingleProjectPost(slug: string): Promise<PostPage> {
    ...
    //Date 型を string に変換しています。これをしないと下記のようなエラーが発生します。
    post = JSON.parse(
      JSON.stringify(NotionService.pageToProjectPostTransformer(page))
    );
    return {
      post,
      markdown,
